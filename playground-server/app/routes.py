...
from services.model_loader import model_cache
...
async def root():
    return {
        ...
        "supported_tasks": ["text-generation", "text-classification", "image-classification", "speech-recognition"]
    }
...
async def run_inference(request: InferenceRequest):
    ...
    logger.info(f"Inference request {request_id}: {request.model_hash} - {request.task}")
    
    try:
        # Validate task
        if request.task not in ["text-generation", "text-classification", "image-classification", "speech-recognition"]:
        ...
        if request.task == "text-generation":
            ...
            if not request.input_text:
                raise HTTPException(status_code=400, detail="input_text is required for text generation")
            ...
            if not request.input_text:
                raise HTTPException(status_code=400, detail="input_text is required for text classification")
            result = await run_text_classification(model_obj, request.input_text, params)
            
        elif request.task == "image-classification":
            # This would require file upload handling - placeholder for now
            raise HTTPException(status_code=501, detail="Image classification not yet implemented - requires file upload")
            
        elif request.task == "speech-recognition":
            # This would require file upload handling - placeholder for now
            raise HTTPException(status_code=501, detail="Speech recognition not yet implemented - requires file upload")
        
        processing_time = (datetime.now() - start_time).total_seconds()
        
        logger.info(f"Inference request {request_id} completed in {processing_time:.2f}s")
    ...
    try:
        ...
        if task == "image-classification":
            ...
            try:
                ...
                results = []
                for score, idx in zip(top_predictions.values.tolist(), top_predictions.indices.tolist()):
            ...
            try:
                ...
                result = pipe(temp_file_path, return_timestamps=params.return_timestamps)
            ...
            raise HTTPException(status_code=400, detail=f"Task {task} does not support file upload")
