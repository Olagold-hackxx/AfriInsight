...
const createZipFromFiles = async (files: File[]): Promise<File> => {
  try {
    ...
    return new File([zipBlob], "upload-archive.zip", { type: "application/zip" })
  } catch (error) {
    console.error("Error creating zip file:", error)
    throw new Error("Failed to compress files. Please try again or upload files individually.")
  }
}
...
export default function UploadPage() {
  ...
  const validateFiles = (newFiles: File[]): { valid: boolean; message: string } => {
    ...
    if (compressedFiles.length > 1) {
      return {
        valid: false,
        message: "Only one compressed file (zip, tar.gz, etc.) is allowed. Please select a single archive.",
      }
    }
    ...
    if (compressedFiles.length === 1 && regularFiles.length > 0) {
      return {
        valid: false,
        message: "Cannot mix compressed files with regular files. Use either one archive or multiple regular files.",
      }
    }
    ...
    onDropRejected: (fileRejections) => {
      ...
      if (rejectedFile) {
        ...
        if (error?.code === "file-too-large") {
          ...
          errorMessage = "File type not supported. Please check the allowed file formats."
        }
  ...
  const prepareFileForUpload = async (files: File[]): Promise<File> => {
    ...
    try {
      ...
      toast.success(`Successfully compressed ${files.length} files into archive`, {
        position: "top-right",
        autoClose: 3000,
      })
  ...
  const uploadContentComplete = async (
    ...
    try {
      ...
      try {
        ...
        const errorMessage = "Failed to upload files to IPFS. Please check your connection and try again."
        toast.error(errorMessage, {
          position: "top-right",
          autoClose: 5000,
        })
      ...
      try {
        ...
        const errorMessage = "Failed to create or upload metadata. Please try again."
        toast.error(errorMessage, {
          position: "top-right",
          autoClose: 5000,
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        toast.success(`🎉 NFT successfully minted! Token ID: ${uploadResult.tokenId}`, {
          position: "top-right",
          autoClose: 8000,
        })
      ...
      if (!errorStr.includes("IPFS") && !errorStr.includes("metadata") && !errorStr.includes("NFT")) {
        toast.error(errorMessage, {
          position: "top-right",
          autoClose: 5000,
        })
  ...
  if (uploadComplete && uploadResult) {
    return (
      <div className="min-h-screen py-8 bg-gradient-to-br from-gray-950 via-slate-900 to-gray-950 text-white flex items-center justify-center">
        <div className="relative z-10 max-w-3xl mx-auto text-center px-6">
          ...
          <p className="text-xl text-slate-300 mb-12 font-light leading-relaxed">
            Your {uploadType} has been uploaded to IPFS and your NFT has been minted. The value will increase as your
            contribution gains popularity.
          </p>
          ...
          <div className="space-4 flex justify-between">
            <Button
              size="lg"
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-16 text-center">
            ...
            <p className="text-xl text-slate-300 max-w-3xl mx-auto font-light leading-relaxed">
              Upload models or datasets to the decentralized network and receive NFTs that appreciate in value based on
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <Card className="bg-slate-900/20 backdrop-blur-sm border-slate-800">
              <CardHeader>
                ...
                <CardDescription className="text-slate-400 font-light">
                  {uploadType === "model"
                    ? "Upload model files (.bin, .safetensors, .pth, .ckpt), config files, and documentation. Multiple files will be automatically compressed."
                  ...
                  {isDragActive ? (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 font-light">
                        {uploadType === "model"
                          ? "Supported: .bin, .safetensors, .pth, .ckpt, .json, .txt, .md, .zip, .tar.gz (Max: 10GB)"
                          : "Supported: .csv, .json, .jsonl, .parquet, .zip, .tar.gz, .txt (Max: 50GB)"}
                      </p>
                      <p className="text-xs text-amber-400 font-light mt-2">
                        💡 Multiple files will be automatically compressed into a single archive
                      </p>
                    </div>
                  )}
                </div>
                ...
                {compressionWarning && (
                  <div className="mt-4 p-4 bg-red-900/20 border border-red-800 rounded-md">
                    <div className="flex items-center">
                      ...
                      <p className="text-red-300 text-sm">{compressionWarning}</p>
                    </div>
                  </div>
                )}
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <span className="text-xs text-slate-400 ml-3">({formatFileSize(file.size)})</span>
                            {isCompressedFile(file) && (
                              <Badge variant="outline" className="ml-3 border-amber-600 text-amber-400 text-xs">
                                Compressed
                              </Badge>
                            )}
                          </div>
              ...
              <CardHeader>
                ...
                <CardDescription className="text-slate-400 font-light">
                  ...
                  <span className="text-amber-400">
                    {" "}
                    Description and metadata are stored on IPFS for richer content.
                  </span>
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <Input
                      id="title"
                      placeholder={
                        uploadType === "model" ? "e.g., GPT-2 Fine-tuned for Poetry" : "e.g., Common Crawl Web Text"
                      }
                ...
                <div className="space-y-3">
                  <Label htmlFor="description" className="text-white font-light">
                    Description *<span className="text-amber-400 text-sm ml-2">(Stored on IPFS)</span>
                  </Label>
                  <Textarea
                    id="description"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    <Label htmlFor="repository" className="text-white font-light">
                      Repository (Optional)
                    </Label>
                    <Input
                      id="repository"
            ...
            <Card className="bg-slate-900/20 backdrop-blur-sm border-slate-800">
              ...
              <CardContent className="space-y-8">
                ...
                {formData.license === "custom" && (
                  <div className="space-y-3">
                    <Label htmlFor="customLicense" className="text-white font-light">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 text-sm">
                      <div
                        className={`flex items-center ${
                          uploadProgressState.stage === "compressing" || uploadProgressState.progress > 10
                            ? "text-green-400"
                            : "text-slate-400"
                        }`}
                      >
                      ...
                      <div
                        className={`flex items-center ${
                          uploadProgressState.stage === "uploading-files" || uploadProgressState.progress > 30
                            ? "text-green-400"
                            : "text-slate-400"
                        }`}
                      >
            ...
            <div className="flex items-center justify-between pt-8 border-t border-slate-800">
              ...
              >
                {isUploading
                  ? "Uploading & Minting..."
                  : `Upload ${uploadType === "model" ? "Model" : "Dataset"} & Mint NFT`}
              </Button>
            </div>
          </form>
        </div>
